...
// ── Separation scratch state (owned by background thread, NOT by Renderer) ──
// This avoids ALL race conditions: the Renderer is untouched during CPU separation.
...
JNIEXPORT jboolean JNICALL Java_com_modelviewer3d_NativeLib_nativeUploadParsed(JNIEnv*,jclass){
    ...
    if(ok){
        // Copy raw data to g_sep* for safe background-thread separation
...
// ── SEPARATION — operates on g_sep* (completely separate from Renderer) ──────
JNIEXPORT jboolean JNICALL Java_com_modelviewer3d_NativeLib_nativePerformSeparationCPU(JNIEnv*,jclass){
    ...
    const uint32_t triCount  = (uint32_t)(g_sepIdx.size() / 3);
    const uint32_t vertCount = (uint32_t)(g_sepVerts.size());
    LOGI("nativePerformSeparationCPU: %u verts, %u tris", vertCount, triCount);

    g_sepComponents.clear();
    g_separator.separate(
        g_sepVerts.data(), vertCount,
        g_sepIdx.data(),   triCount,
        g_sepComponents);
...
JNIEXPORT jboolean JNICALL Java_com_modelviewer3d_NativeLib_nativePerformSeparationGPU(JNIEnv*,jclass){
    if(!g_renderer || g_sepComponents.empty()) return JNI_FALSE;
...
JNIEXPORT jboolean JNICALL Java_com_modelviewer3d_NativeLib_nativeIsSeparated(JNIEnv*,jclass){
    return (g_renderer && g_renderer->isSeparated()) ? JNI_TRUE : JNI_FALSE;
}
...
JNIEXPORT jint JNICALL Java_com_modelviewer3d_NativeLib_nativeGetMeshVertexCount(JNIEnv*,jclass,jint idx){
    return g_renderer ? (jint)g_renderer->getMeshVertexCount((int)idx) : 0;
}
